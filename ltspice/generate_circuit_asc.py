...
def build_circuit():
    create_directory()
    dim = config.layer_dims[0]
    input_voltages = ['']*(dim[0]+config.num_of_layers -1)
    ...
    while i < config.num_of_layers:
        # Getting Layer dimensions
        dim = config.layer_dims[i]
        ...
        with open(data_directory + 'resistances_lyr{lyr_num}.csv'.format(lyr_num = i+1), newline='') as csvfile:
                ...
                for row in spamreader:
                        resistances[itr,] = row
                        itr += 1
        if i == 0 and config.num_of_layers == 1:
                lyr = nn_lyr.nnlayer(i,dim[0],dim[1], resistances, 'relu', input_voltages)
                lyr.isFirst = True
                lyr.hasNext = False
                lyr.hasPrveious = False
        elif i == 0 and config.num_of_layers > 1:
                lyr = nn_lyr.nnlayer(i,dim[0],dim[1], resistances, 'relu', input_voltages)
                lyr.isFirst = True
                lyr.hasNext = True
                lyr.hasPrveious = False
        elif i == config.num_of_layers - 1 and config.num_of_layers > 1:
        ...
        current_vlocation += config.block_vspacing
    ...
    stment += 'TEXT -1008 -272 Left 2 !.include ../components/180nm_bulk.txt \n'

    write_to_ascfile(stment)
    return config.spice_directory + file_name + '.asc'

def write_to_ascfile(stment):
    circuit_file_name = config.spice_directory + file_name + '.asc'
    with open(circuit_file_name,'w') as nf :
        nf.write(stment)
    return

def create_directory():
     subfolders= [f.path for f in os.scandir(config.spice_directory) if f.is_dir() and f.name.startswith(config.wd_prefix) ]
     gwd = '{wdp}{idx}'.format(wdp = config.wd_prefix, idx = len(subfolders))
     wd = '{parent}{gwd}/'.format(parent = config.spice_directory, gwd = gwd)
     #os.mkdir(wd)
     config.spice_directory = wd
     config.LTSpice_asc_filename = config.spice_directory + 'complete_circuit.asc'
     # copying sub cells
     shutil.copytree(config.required_directory, wd)
