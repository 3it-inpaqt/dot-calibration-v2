...
if __name__ == '__main__':
    ...
    # Training and tuning
    run_tasks_planner(tune_all, skip_existing_runs=True, tuning_task=True)

    # Full scan for each diagram for the first seed
    run_tasks_planner(full_scan_all, skip_existing_runs=True, tuning_task=True)

    # Uncertainty study
    run_tasks_planner(uncertainty_study_all, skip_existing_runs=True, tuning_task=False)
    ...
    # Full scan for each diagram for the first seed
    run_tasks_planner(full_scan_cross_valid, skip_existing_runs=True, tuning_task=True)

    # Uncertainty study
    run_tasks_planner(uncertainty_study_cross_valid, skip_existing_runs=True, tuning_task=False)
